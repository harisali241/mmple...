...
<div class="container">
	<div class="row">
			<div class="col-md-8">
				<ol class="breadcrumb">
				  <li><a href="{{ url('privateShowtime') }}">Home</a></li>
	...
	<div class="row">
		<div class="col-md-12">
			<div class="form-header-inner">
				...
				@if(in_array('privateShowTime.create', getRoutes()))<a class="pull-right add-button" href="{{ url('privateShowTime/create') }}">Add Show time </a>@endif
			</div>
		</div><!--col-md-12-->
	</div><!--row-->
			...
			@if(count($showTimes) > 0)
				<table border="1" cellpadding="0" id="example" cellspacing="0" class="table table-hover tableView admin-table">
					...
					<tr>
						...
						<th>Show Timings</th>
						<th>Status</th>
						@if(in_array('privateShowTime.edit', getRoutes()))<th></th>@endif
						@if(in_array('privateShowTime.destroy', getRoutes()))<th></th>@endif
					...
					<tbody class="searchable">
						...
						@foreach($showTimes as $showTime)
							<tr>
								...
								@if(in_array('privateShowTime.edit', getRoutes()))<td class="alignCenter"><a class="edit_btn" href="{{ url('privateShowTime/'.$showTime->id.'/edit') }}">Edit</a></td>@endif
								@if(in_array('privateShowTime.destroy', getRoutes()))
								<td class="alignCenter">
									<form action="{{ url('privateShowTime/'.$showTime->id) }}" method="post" id="delete{{$showTime->id}}">
				                   		{{ csrf_field() }}
				                   		{{method_field("DELETE")}}
